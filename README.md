...
| 탭 | 기능 |
|----|------|
| **Overview** | 에이전트 상태 카드 + Chronicle(태스크 목록) + 실시간 로그 스트림 |
| **Terminals** | 에이전트별 터미널 뷰 (Odin/Brokkr/Heimdall 로그 분리) |
| **Skills** | 12개 내장 Skill 카탈로그 (카테고리 필터, 상세 정보) |
...
| 파일 감시 | Chokidar (INDEX.md, 로그 파일, PID 파일 실시간 추적) |
| 실시간 통신 | WebSocket (`/ws/logs`, `/ws/status`) |
...
src/yggdrasil/
├── server/
│   ├── index.ts          # Express + Next.js + WebSocket 서버
│   ├── routes.ts         # REST API (/api/status, /api/chronicle, /api/document)
...
│   ├── components/
│   │   ├── Header.tsx        # 상단 바 (연결 상태)
│   │   ├── AgentCard.tsx     # 에이전트 상태 카드
│   │   ├── Chronicle.tsx     # 태스크 목록 (클릭 시 TP/RP 뷰어)
│   │   ├── LiveLog.tsx       # 실시간 로그 스트림
│   │   ├── TerminalPanel.tsx # 에이전트별 터미널 뷰
│   │   ├── SkillsPanel.tsx   # Skill 카탈로그
│   │   └── DocViewer.tsx     # TP/RP 마크다운 뷰어 (슬라이드 패널)
│   └── lib/
